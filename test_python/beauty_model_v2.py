...
def train_model(FLAGS):
    ...
    with tf.Session(config=config) as sess, open(log_path, 'w') as writer:
        ...
        for step in range(FLAGS.max_iter):
            ...
            if (step % 500 == 0) or (step >= 10000 and (step + 1) % FLAGS.valid_freq == 0):
                ...
                writer.write('iter{}, av_rmse:{:.4f}, av_peason:{:.4f}\n'.
                             format(step, rmse_average, pearson_average))
                writer.flush()

                if pearson_average >= 0.90:
